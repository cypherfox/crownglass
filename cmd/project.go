...
import (
	...
	"github.com/cypherfox/crownglass/tpl"
...
func (p *Project) Create() error {
	...
	err = p.makeCommand("server", "rootCmd").Create()
	if err != nil {
		return err
	}

	err = p.makeCommand("service", "rootCmd").Create()
	if err != nil {
		return err
	}

	err = p.makeCommand("frontend", "rootCmd").Create()
	if err != nil {
		return err
	}

	err = p.makeCommand("page", "frontendCmd").Create()
	if err != nil {
		return err
	}

	err = p.makeCommand("css", "frontendCmd").Create()
	if err != nil {
		return err
	}

	err = p.makeCommand("js", "frontendCmd").Create()
	if err != nil {
		return err
	}
...
func (p *Project) makeCommand(name, parent string) *Command {
	return &Command{
		CmdName:   name,
		CmdParent: parent,
		Project:   p,
...
func (c *Command) Create() error {
	return c.CreateWithTemplate(tpl.AddCommandTemplate())
}

func (c *Command) CreateWithTemplate(tmpl []byte) error {
	...
	commandTemplate := template.Must(template.New("sub").Parse(string(tmpl)))
